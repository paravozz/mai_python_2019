...
# Как должна работать программа: на вход даете файл с текстом песни, на выходе
# получаете самые часто встречающиеся слова.
# Короткие слова (предлоги, союзы, метоимеия и пр.) лучше отфильтровать сразу
...
# или, что то же самое, - оставить только буквы русского и латинского алфавита,
# знак "тире" и пробел
...
for pair in words_count:
	if len(pair[1]) > 3:
		# добавьте в словарь words_dict значение pair[1]
		# в список по ключу pair[0]
		# не забудьте проверить, существует ли у вас список для этого ключа,
		# и если нет - создать его удобным для вас способом,
		# как мы делали для квартир
		key = pair[0]
		value = pair[1]
...
middle = len(keys) // 2
# выведите результат
for key in sorted(keys, reverse=True)[0: middle]:
